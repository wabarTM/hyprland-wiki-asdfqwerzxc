...
> 
> For live event handling, see the [events](../10-expanding-functionality/#events).
...
See [Dispatchers](../../40-dispatchers) for a list of dispatchers.
...
Where `window` is as described [here](../../40-dispatchers#parameter-explanation), and `property` is any which can be set with [set_prop](../../40-dispatchers/#set_prop).
...
See [Config options](../../20-config-options) for sections and options you can use.

...
| config.props_refreshed | Emitted when a prop refresh event is executed. | Bool: Is the [prop refresh](../lua-utilities/#prop-refresh) event executed as scheduled (`false` if executed prematurely with helper function) |

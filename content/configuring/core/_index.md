---
weight: 10
title: Core
---

The config is located in `$XDG_CONFIG_HOME/hypr/hyprland.lua`. In most cases,
that maps to `~/.config/hypr/hyprland.lua`.

When using start-hyprland, arguments to Hyprland are passed after `--`.
Use `--config` (or `-c`) to make Hyprland load specified config.

If no configuration exists, Hyprland will create
an [example config](https://github.com/hyprwm/Hyprland/blob/main/example/hyprland.lua)

Remove `hl.config({ autogenerated = true })` to remove autogenerated warning.

Configuration is realoaded when changes to files are detected. To reload manually,
use `hyprctl reload`.

> [!WARNING]
> The default config is not complete and does not include every Hyprland options or feature.
> For every configuration option, use the pages linked on this wiki section.

## Language style and syntax

Hyprland uses [Lua](https://lua.org/) for its configuration file. It's an easy
and performant scripting language used throughout various pieces of software.

### General Hyprland Lua tips

#### Example config

Example config file can be found [here](https://github.com/hyprwm/Hyprland/blob/main/example/hyprland.lua).

#### Using Multiple Configuration Files

Configuration can be split across several files.
To load other files, use Lua's [require](#require) function.
Files may be specified either as an absolute path or a path relative to the location of `hyprland.lua`.

In `require`, `.` or `/` can be used as a directory separator.

The `*` wildcard can be used to load multiple files from a directory or to
search in all directories at the given path. When wildcard is used path must be in
UNIX format.

{{% details title="Examples" closed="true" %}}

```lua
-- refers to $XDG_CONFIG_HOME/hypr/awesomeconf/keybinds.lua
require("awesomeconf/keybinds")

-- refers to $XDG_CONFIG_HOME/hypr/awesomeconf/animation.lua
require("awesomeconf.animation")

-- will try to load all files in lua derictory
require("./lua/*")
-- or
require("../hypr/lua/*")
-- or
require("./lua/*/loadme.lua")

-- will load "us" file from /usr/share/among
require("/usr/share/among/us")
```

{{% /details %}}

#### Require

Use `require` to load other files into your config. This is **highly** recommended
because each `require` call is specifically made by Hyprland to be a separate
lua "scope", so errors in one `require`d file do _not_ stop execution of other files.

It's important to note that many errors will kill the execution of a given lua file.
Error behavior is described further below.

`require`'s protective behavior does not apply to module loading. If you
attempt to load a module that does not exist, `require` will throw an
actual error, in the file from which it was invoked. That is, doing
`require("nonexistent")` in your main Hyprland config _will kill the
execution of your main config_ as described below.

If you want to prevent such errors from breaking stuff, or even detect and
respond to them at runtime, use lua's `pcall()` function, for example:
```lua
local status, value = pcall(require, "maybe-nonexistent")
if status then
  print("successfully loaded module, it returned:", value)
else
  print("failed to load module, its error message was:", value)
end
```

##### Ignore require protection

To ignore scope separation, the original Lua `require` is available as `__require`.
It can be used directly, or restored as the default by setting `require = __require`.
Restored functionality may be helpful if you load third-party modules that are broken
by Hyprland's error-protection behavior.

#### Autocompletions

Pre-generated Lua stub can be found in `/usr/share/hypr/stubs/` directory. Configure your LSP
to include that directory in your Lua workspace to enable autocompletions.

{{< tabs >}}

{{< tab name="Generic" >}}

Add a `.luarc.json` file in your repository root, then:

```json
{
  "workspace": {
    "library": [
      "/usr/share/hypr/stubs"
    ]
  }
}
```

{{< /tab >}}

{{< tab name="VS Code" >}}

Create a `settings.json` file in `.vscode/` (if it doesn't exist, make it), then:

```json
{
  "Lua.workspace.library": [
    "/usr/share/hypr/stubs"
  ]
}
```

{{< /tab >}}

{{< /tabs >}}

#### Error behavior

Hyprland attempts as much as it can to make errors as non-destructive as possible. However,
it's not always possible:
 - Fundamental Lua syntax errors will make Hyprland refuse to reload your config and pop an error.
 - Runtime Lua syntax errors will abort execution of the current Lua file and pop an error (e.g. calling a `nil`).
 - Runtime Hyprland type errors will continue execution and pop an error (e.g. passing a string instead of a float to `hl.*`).
 - Runtime errors during async execution (e.g. a keybind function) will pop a notification about the error.

If an errors occurs before any of the binds were registered, Hyprland will register a few
emergency keybinds: `SUPER + Q` to open terminal, `SUPER + R` to open hyprlauncher, and `SUPER + M` to exit Hypland, and mention this in the error popup.

#### Infinite loops, reentrant events

Hyprland has protections in place to avoid scripts that would run forever from taking down
the session. These will kill your script after a given timeout / limit has been reached.

#### Standard library

The Lua standard libraries are loaded by default. Lua scripts can execute
arbitrary code on your machine, make sure you trust your config source.

#### Testing with REPL

The API and Lua state can be easily explored with the built in [Lua REPL in hyprctl](./advanced-configuration/using-hyprctl/#repl).
